// ********************************************
// filename: CommonStuff.scad
//  by Dave Flynn 2013
// This file contains constants and some common routines
// Revision: 1.0 11/23/2017 Consolidated.
// Units: millimeters
// ********************************************
// History
// 1.0 11/23/2017 Consolidated.
// 4/30/2015  Added Bolt(Len=10)
// 12/29/2013 Added BoltHole6(depth=rPin*3)
//
// ********** Constants **********
rIDInc=0.1; // radius Inside Diameter Increment, material dependant recommended value 0.1 to 0.25
...
rBoltClear8=2.15; // #8-32 clearance hole radius
...
//Bolt(Len=10);					// #4-40 Bolt (for viewing)
...
//BoltClearHole8(depth=rPin*3); // #8-32 thread clearance hole
...
module Bolt(Len=10){
	difference(){
		union(){
			cylinder(r=rEndPinBoltHead-0.2, h=hEndPinBoltHead);
			translate([0,0,hEndPinBoltHead-0.05]) cylinder(r=rEndPinBolt, h=Len);
		}
		translate([0,0,-0.05]) cylinder(r=1.3,h=2.5,$fn=6);
	}
...
module BoltClearHole8(depth=rPin*3){
	translate([0,0,-depth-0.05])
		cylinder(r=rBoltClear8,h=depth+0.1,$fn=24);
	//translate([0,0,-hEndPinBoltHead])
		//cylinder(r=rEndPinBoltHead,h=depth+0.1,$fn=24);
} // BoltClearHole8
...
module ServoHolesHS5245MG(ExtraH=0){
	// Modfied 11/1/15 #4 screws
	servoTray_h=6;
	...
	servoBC_w=8;
		...
		translate([sevroBC_l/2,servoBC_w/2+0.5,0]) BoltHole2(depth=10);
...
module ServoMountHS5245MG(ExtraH=0){
	...
	difference(){
		...
		translate([sevroBC_l/2,servoBC_w/2+0.5,0]) BoltHole(depth=10);
		translate([-sevroBC_l/2,servoBC_w/2,0]) BoltHole(depth=10);
